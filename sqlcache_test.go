...
func TestNoCache(t *testing.T) {
	...
	sqlCacheDriver := NewSqlCacheDriver(&mockDriver, &sqlCacher, log)
	sql.Register("mock-cache", &sqlCacheDriver)
	db, err := sql.Open("mock-cache", "")
	if err != nil {
	...
	defer db.Close()
	defer memCache.ClearAll()

	mockConn.Rows = &MockRows{
		Cols:     []string{"number"},
		Err:      nil,
		NextVals: []driver.Value{5},
	}
